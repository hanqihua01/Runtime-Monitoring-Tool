<template>
    <div>
        ...
        <div>
            <h1>当前虚拟内存大小：</h1>
            <p>虚拟内存大小低于<label><input v-model="warningThreshold" style="width: 100px; text-align: center;"></label>时，为<span
                    style="color: #057b29">OK</span>状态；
                虚拟内存大小不低于<label><input v-model="warningThreshold" style="width: 100px; text-align: center;"></label>时，为<span
                    style="color: #f8b303">WARNING</span>状态；
                虚拟内存大小不低于<label><input v-model="criticalThreshold"
                        style="width: 100px; text-align: center;"></label>时，为<span style="color: #f04949">CRITICAL</span>状态。
            </p>
            <el-table :data="tableData" style="width: 100%" :row-class-name="tableRowClassName">
                <el-table-column prop="time" label="时间" sortable>
                </el-table-column>
                <el-table-column prop="num" label="虚拟内存大小" sortable>
                </el-table-column>
...
export default {
    data() {
        return {
            warningThreshold: 105000000, // 虚拟内存大小警告阈值
            criticalThreshold: 110000000, // 虚拟内存大小风险阈值
    ...
    created() {
        const _this = this
        const getInfo = () => {
            axios.get('http://localhost:3000/VirtualMemorySize')
                .then(response => {
